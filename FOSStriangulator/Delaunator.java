...
public class Delaunator {
    float[] coords;
    float EPSILON = (float)Math.pow(2.0f, -22); //todo double-check this is correct
    ...
    int maxTriangles;
    private int[] _triangles = new int[maxTriangles * 3];

    private int[] _halfedges = new int[maxTriangles * 3];

    int[] triangles = null;
    int[] halfedges = null;

    // temporary arrays for tracking the edges of the advancing convex hull
    private int hashSize = (int) Math.ceil(Math.sqrt(count * 1.0));

    private int[] hullPrev = new int[count]; // edge to prev edge

    private int[] hullNext = new int[count]; // edge to next edge

    private int[] hullTri = new int[count]; // edge to adjacent triangle
    ...
    Delaunator(float[] coords) {
        ...
        ids = new int[count];
        maxTriangles = 2 * count - 5 >= 0 ? 2 * count - 5 : 0;
        update();
    }
    ...
    void update() {
        ...
        hullNext[i0] = i1; // todo out of bounds exception here
        hullNext[i1] = i2;
        hullNext[i2] = i0;
    ...
    // robustness issues)
    private float orient(float rx, float ry, float qx, float qy, float px, float py) {
